...
export default function MessagesPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || t('toastMessageSendError'));
    }
  };
  ...
  const deleteMessage = async (msgId: string, mode: 'me' | 'all') => {
    ...
    try {
      ...
      toast.success(t('toastMessageDeleted'));
  ...
  const handleBlock = async () => {
    if (!otherUser) return;
    if (!confirm(t('confirmBlock', { name: otherUser.firstName }))) return;
    try {
      await api.post('/messages/block', { blockedId: otherUser.id });
      toast.success(t('toastBlocked')); setShowMenu(false);
    } catch { toast.error(t('toastError')); }
  };
  ...
  const submitReport = async () => {
    if (!reportReason) { toast.error(t('toastChooseReason')); return; }
    try {
      await api.post('/reports', { reason: reportReason, reportedUserId: otherUser?.id });
      toast.success(t('toastReportSent'));
                  ...
                  return (
                    <Link key={conv.id} href={`/messages/${conv.id}`} className={`flex items-center gap-3 p-4 hover:bg-dark-50 transition-colors border-b border-dark-50 ${conv.id === conversationId ? 'bg-primary-50' : ''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-dark-400 text-xs truncate">{conv.lastMessage || t('startConversation')}</p>
                      </div>
                      {conv.unreadCount > 0 && (
            ...
            <div className="flex-1 flex flex-col">
              {conversationId ? (
                <>
                  <div className="p-4 border-b border-dark-100 flex items-center gap-3 relative">
                    ...
                    <div className="flex-1">
                      <p className="font-semibold text-dark-900">{otherUser ? `${otherUser.firstName} ${otherUser.lastName}` : t('unknownUser')}</p>
                      {typing ? (
                        <p className="text-xs text-primary-600 animate-pulse">{t('typing')}</p>
                      ) : (
                    ...
                    {showMenu && (
                      <div className="absolute top-16 right-4 bg-white rounded-2xl shadow-card-hover border border-dark-100 py-2 w-56 z-50">
                        <Link href={`/profil/${otherUser?.id}`} className="flex items-center gap-3 px-4 py-2.5 text-sm text-dark-700 hover:bg-dark-50 transition-colors"><User size={16} /> {t('viewProfile')}</Link>
                        <button onClick={() => { setShowReport(true); setShowMenu(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-dark-700 hover:bg-dark-50 transition-colors"><Flag size={16} /> {t('report')}</button>
                        <button onClick={handleBlock} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-orange-600 hover:bg-orange-50 transition-colors"><Ban size={16} /> {t('block')}</button>
                        <button onClick={handleDelete} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-guinea-500 hover:bg-guinea-50 transition-colors"><Trash2 size={16} /> {t('deleteConversation')}</button>
                      </div>
                    )}
                  </div>
                  ...
                  {activeConv?.annonce && (
                    ...
                    >
                      ...
                      <span className="text-primary-400 group-hover:text-primary-600 text-xs font-medium shrink-0">{t('seeListing')}</span>
                    </Link>
                  )}
                    ...
                    <div className="mx-4 mt-2 mb-1 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5">
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs font-bold text-amber-800">{t('safetyTipsTitle')}</span>
                        </div>
                      ...
                      <ul className="text-[11px] text-amber-800 space-y-1">
                        <li className="flex items-start gap-1.5"><span className="font-bold text-amber-500 shrink-0">!</span> {t('safetyTip1')}</li>
                        <li className="flex items-start gap-1.5"><span className="font-bold text-amber-500 shrink-0">!</span> {t('safetyTip2')}</li>
                        <li className="flex items-start gap-1.5"><span className="font-bold text-amber-500 shrink-0">!</span> {t('safetyTip3')}</li>
                        <li className="flex items-start gap-1.5"><span className="font-bold text-amber-500 shrink-0">!</span> {t('safetyTip4')}</li>
                      </ul>
                    </div>
                  )}
                          ...
                          <div className="relative max-w-xs lg:max-w-md">
                            <div className={`px-4 py-2.5 rounded-2xl text-sm ${isMe ? 'bg-primary-700 text-white rounded-br-sm' : 'bg-dark-100 text-dark-900 rounded-bl-sm'} ${msg.deletedForAll ? 'italic opacity-60' : ''}`}>
                              ...
                              {msg.replyTo && !msg.deletedForAll && (
                                <div className={`text-xs mb-1.5 px-2 py-1 rounded-lg border-l-2 ${isMe ? 'bg-primary-800/40 border-gold-300' : 'bg-dark-200/60 border-primary-500'}`}>
                                  ...
                                  <p className="truncate opacity-80">{msg.replyTo.content || t('imagePlaceholder')}</p>
                                </div>
                              )}
                              {msg.deletedForAll ? (
                                <p className="flex items-center gap-1"><Ban size={13} /> {t('messageDeleted')}</p>
                              ) : (
                                <>
                              ...
                              <div className={`flex items-center gap-1 mt-1 ${isMe ? 'justify-end text-primary-200' : 'text-dark-400'}`}>
                                <span className="text-xs">{formatDistanceToNow(new Date(msg.createdAt), { addSuffix: true, locale: DATE_LOCALES[locale] ?? fr })}</span>
                            ...
                            {msgMenu === msg.id && (
                              <div className={`absolute top-6 ${isMe ? 'left-0' : 'right-0'} bg-white rounded-xl shadow-card-hover border border-dark-100 py-1.5 w-48 z-50`}>
                                <button onClick={() => { setReplyTo(msg); setMsgMenu(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-dark-700 hover:bg-dark-50"><Reply size={14} /> {t('reply')}</button>
                                <button onClick={() => deleteMessage(msg.id, 'me')} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-dark-700 hover:bg-dark-50"><Trash2 size={14} /> {t('deleteForMe')}</button>
                                {isMe && <button onClick={() => deleteMessage(msg.id, 'all')} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-guinea-500 hover:bg-guinea-50"><Trash2 size={14} /> {t('deleteForEveryone')}</button>}
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  {replyTo && (
                    <div className="px-4 py-2 border-t border-dark-100 bg-dark-50 flex items-center gap-3">
                      <div className="flex-1 border-l-2 border-primary-500 pl-3">
                        <p className="text-xs font-semibold text-primary-700">{t('replyTo', { name: replyTo.sender?.firstName })}</p>
                        <p className="text-xs text-dark-500 truncate">{replyTo.content || t('imagePlaceholder')}</p>
                      </div>
                  ...
                  <div className="p-4 border-t border-dark-100">
                    <div className="flex items-center gap-2">
                      <input value={input} onChange={handleTyping} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder={t('inputPlaceholder')} className="flex-1 bg-dark-50 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary-500" />
                ...
                <div className="flex-1 flex items-center justify-center">
                  <div className="text-center"><MessageCircle size={60} className="text-dark-200 mx-auto mb-4" /><p className="text-dark-500 font-medium">{t('selectConversation')}</p></div>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowReport(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-bold text-dark-900 text-lg">{t('reportUserTitle')}</h3>
              <button onClick={() => setShowReport(false)} className="text-dark-400 hover:text-dark-700"><X size={20} /></button>
            </div>
            <div className="space-y-2 mb-4">
              {[
                { v: 'SCAM',                 l: t('reasonScam'),         Icon: AlertTriangle },
                { v: 'SPAM',                 l: t('reasonSpam'),         Icon: Mail },
                { v: 'INAPPROPRIATE_CONTENT',l: t('reasonInappropriate'),Icon: AlertOctagon },
                { v: 'OTHER',                l: t('reasonOther'),       Icon: HelpCircle },
              ].map(r => (
            ...
            <button onClick={submitReport} className="w-full bg-guinea-500 text-white font-semibold py-3 rounded-xl hover:bg-guinea-600 transition-colors">{t('submitReport')}</button>
          </div>
        </div>
      )}
    </div>
