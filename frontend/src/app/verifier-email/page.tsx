...
function VerifierEmailContent() {
  const t = useTranslations('auth.verifierEmail');
  ...
  useEffect(() => {
    if (!token) {
      setStatus('error');
      setMessage(t('incompleteLinkMsg'));
      return;
    }
    api.post('/auth/verify-email', { token })
      .then((res) => {
        setStatus('success');
        setMessage(res.data.message || t('successDefaultMsg'));
      ...
      .catch((err) => {
        setStatus('error');
        setMessage(err.response?.data?.error || t('genericErrorMsg'));
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.success(res.data.message || t('toastEmailSent'));
        ...
        <div className="w-full max-w-md">
          ...
          {status === 'loading' && (
            <div className="text-center">
              ...
              <h2 className="font-display font-bold text-2xl text-dark-900 mb-3">{t('loadingTitle')}</h2>
              <p className="text-dark-500 text-sm">{t('loadingSubtitle')}</p>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="text-center">
              ...
              >
                {isAuthenticated ? t('goToProfile') : t('login')}
              </Link>
            </div>
          )}
          ...
          {status === 'error' && (
            <div className="text-center">
              ...
              <h2 className="font-display font-bold text-2xl text-dark-900 mb-3">
                {expired ? t('linkExpiredTitle') : t('linkInvalidTitle')}
              </h2>
              <p className="text-dark-500 text-sm mb-6 leading-relaxed">{message}</p>
              ...
              <div className="bg-dark-50 rounded-2xl p-5 text-left">
                <p className="text-sm font-semibold text-dark-800 mb-3 flex items-center gap-2">
                  <Mail size={16} className="text-primary-700" /> {t('resendTitle')}
                </p>
                {!isAuthenticated && (
                  <input
                    type="email"
                ...
                >
                  {resending
                    ? <><Loader2 size={16} className="animate-spin" /> {t('resending')}</>
                    : resendCooldown > 0
                      ? t('retryIn', { seconds: resendCooldown })
                      : t('resendButton')}
                </button>
              </div>
